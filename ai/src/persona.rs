/*
 ...
 past and present dedicated explorers from this plane of No-Conflict consciousness:
  ...
  DISCOVERED and SHARED. The "IamX" is cultivable via reversed engineering observable 
...
/* 
  ...
  levels of security: (1): container, (2): container-2-container, (3): certified source,
  (4): auditted legal requirements, (5): hardware Ids, (6): trusted, (7): validated Trust, 
  ...
  The db_map_container belongs the owner having its unique relationships Ids and the
  private/public IPv6, PLUS firmware code standardization of #EmptyTheContent toward 
  Non-Thingness / Nothingness in the owner scientifically cultivable #Prajna in the 
  No-Conflict Consciousness orthogonal to the duality planes of conflicting consciousness. 
  The value of key "owner / स्वामी" is Vec v of unique Array of bytes: &v[0] = onwer id, &v[1] 
  = onwer hardware id, &v[2] = onwer certified security level. Its relationship key id has
  &v[0] = relationship with the owner, &v[1] = the persona hardware ids, &v[2] = persona
  certified security level. Additional values are two possible related realms the "owner / 
   स्वामी" may be naturally reincarnated where the distributed records are securely maintained, 
  waiting for its legitimate activation based on our empirically Know-How to avoid past 
  mistakes hard-learned from 60,000+ human-years of esoteric degenerated feudal systems. 
  Security features can be physically custom designed and implemented at chip level. Only 
  containers belong to those having RoT (Root of Trust) can communicate and collaborate 
  within a certified boundary.
*/
...
fn build_inner_space (_x: Vec<i32>, _y: Vec<i32>, _f: Vec<i32>) -> InnerSpace  {
  ...
  y.insert(5, String::from("Y+5: NonThingness"));  // = 5  Gotama's impass-SelfSelfless
  ...
  y.insert(-6, String::from("Y-6: Vampire"));  // = -8 unqualified at humanitas
  ...
  f.insert(-7, String::from("F-7: Smelly2"));                  // =  13   Only im esoteric
...
/* 
  Pointer EmptyTheContent toward #NonThingness - sufficient #KindnessEmpathy
  Tranquility Swiss = 1, Equanimity enforceable HOC 2 = 1, Purity Resort 3 = 2,
  Selfless 4 = 3, NonThingness / NonThingness 5 = 8
*/
...
/*
  ...
  security levels. In each IamX, higher security can access lower levels but Not reversed
...
} // end of build_InnerSpace from _t, _p, _y, _x, _f
...
/*
  ...
  Much deeper researches are required in this era.
...
capabilities. Reference pointer type enables you to allow data to have multiple owners 
...
*/
fn p_evaluation(p: HashMap<i32, String>) -> Vec<i32> { // return evaluated value to blockch
